...
class Shape():
    ...
    def __init__(self, position, velocity, angle, angular_velocity, can_move, is_inverted, moment, mass):
        """
...
class Circle(Shape):
    ...
    Attributes:
        ...
        _can_move: A boolean representing if the shape can move or not.
    """
    def __init__(self, radius, position, velocity=Vector(0, 0), angle=0, angular_velocity=0, can_move=False, is_inverted=False):
        """
        Initialize radius, position, velocity, angle, angular_velocity, can_move, and is_inverted. Calculate mass and moment.
        ...
        self._radius = radius
        super.__init__(self, position, velocity, angle, angular_velocity, can_move, is_inverted, moment, mass)
...
class Polygon(Shape):
    ...
    Attributes:
        ...
        _radius: A float representing the distance between the center of mass and the furthest part of the polygon from the center of mass.
        _is_inverted: A boolean representing if the mass of the polygon is outside of it.
        ...
        _can_move: A boolean representing if the shape can move or not.
    """
    def __init__(self, vertices, position, velocity=Vector(0, 0), angle=0, angular_velocity=0, can_move=False, is_inverted=False):
        """
        ...
        """
        # Calculate mass and moment
        segment_areas = [vertices[i-1].x * vertices[i].y - vertices[i-1].y * vertices[i].x for i in range(len(vertices))]
        mass = abs(sum(segment_areas)) / 2
        x_com = sum([(vertices[i-1].x + vertices[i].x) * segment_areas[i] for i in range(len(vertices))]) / (6 * mass)
        y_com = sum([(vertices[i-1].y + vertices[i].y) * segment_areas[i] for i in range(len(vertices))]) / (6 * mass)
